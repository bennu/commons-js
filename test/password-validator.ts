...
describe('Password Validator', () => {
  ...
  // Test data: edge case passwords
  const edgeCasePasswords = [
    'Aa1!', // Minimum length but shorter than default
    'A'.repeat(12) + 'a1!', // Exactly minimum length
    'A'.repeat(64) + 'a1!', // Over maximum length
    'MyPass  123!', // With spaces
    'MyPass\t123!', // With tab
    'MyPass\n123!', // With newline
    'Aa1!' + 'x'.repeat(60), // Exactly 64 chars
    'password', // Common word
    '123456', // Common sequence
    'qwerty123!A', // Keyboard pattern
    'aaaaA1!aaaa', // Repeated chars
    'abcA1!defgh' // Sequential pattern
  ]
  ...
  describe('calculatePasswordStrength', () => {
    ...
    test('should penalize common patterns', () => {
      const normalScore = calculatePasswordStrength('MySecureP@ssw0rd123')
      const commonScore = calculatePasswordStrength('password123')
      const sequentialScore = calculatePasswordStrength('abc123DEF!')
      const repeatedScore = calculatePasswordStrength('aaaaA1!aaaa')
      
      expect(normalScore).toBeGreaterThan(commonScore)
      expect(normalScore).toBeGreaterThan(sequentialScore)
      expect(normalScore).toBeGreaterThan(repeatedScore)
    })

    test('should respect custom configuration', () => {
      const customConfig = { maxLength: 20 }
      const score1 = calculatePasswordStrength('MySecureP@ssw0rd123', customConfig)
      const score2 = calculatePasswordStrength('MySecureP@ssw0rd123')
  ...
  describe('validatePasswordStrength', () => {
    ...
    test('should validate all check properties', () => {
      const result = validatePasswordStrength('MySecureP@ssw0rd123')
      expect(result.checks).toHaveProperty('hasMinLength')
      expect(result.checks).toHaveProperty('hasMaxLength')
  ...
  describe('passwordsMatch', () => {
    ...
    test('should handle non-string inputs', () => {
      ...
      nonStringInputs.forEach((input) => {
        expect(passwordsMatch(input, input)).toBe(true)
      })
    })
  })
  ...
  describe('createStrongPasswordValidator', () => {
    ...
    test('should handle undefined/null control values', () => {
      const validator = createStrongPasswordValidator()
      const mockControl1 = { value: null }
      const mockControl2 = { value: undefined }
      
      expect(validator(mockControl1)).not.toBe(null)
      expect(validator(mockControl2)).not.toBe(null)
    })
  })
  ...
  describe('createPasswordMatchValidator', () => {
    ...
    test('should handle missing form controls', () => {
      const validator = createPasswordMatchValidator('pass1', 'pass2')
      const mockFormGroup = {
        get: () => null
      }
